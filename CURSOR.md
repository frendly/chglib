...
- **Расположение**: `pages/BENex/YYYY/*.html`
- **Расписание обновлений**: Каждую среду
- **Формат файлов**: HTML (passthrough, не обрабатываются шаблонами)
- **Структура**: `BENexNN.html` где NN = 01-21 (номер бюллетеня)
- **Пример**: `pages/BENex/2025/BENex04.html`
...
**Именование:**
- Формат: `BENexNN.html` где NN = 01-21
- Пример: `BENex04.html`
- HTML passthrough (не обрабатываются шаблонами)
...
1. **Node.js версия**: Требуется >=20 (рекомендуется 22.x, указано в `.nvmrc`)
2. **Yarn версия**: Зафиксирована в `package.json:35` (1.22.22)
3. **Формат дат**: Новости должны иметь формат `YYYY-MM-DD.md` для автоматической обработки
4. **LEGACY папка**: `src/LEGACY/` не используется в сборке, только для справки
5. **Production сборка**: Всегда использует `NODE_ENV=production` для минификации
6. **Watch режим**: Автоматически отслеживает изменения в `src/assets/`
