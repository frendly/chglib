...
/**
 * Fix line breaks inside <b> tags
 * Replaces newlines and multiple spaces with a single space
 * Trims leading and trailing whitespace inside tags
 */
function fixBoldLinebreaks(html: string): { content: string; modified: boolean } {
  let modified = false;

  const fixed = html.replace(/<b>([\s\S]*?)<\/b>/g, (_match, content: string) => {
    // Normalize the content inside <b> tags
...
function processFile(filePath: string, dryRun = false): { success: boolean; modified: boolean } {
  try {
    ...
    // Fix bold linebreaks in the body
    const { content: fixedBody, modified } = fixBoldLinebreaks(body);
...
function main() {
  ...
  console.log('Fix Bold Tag Line Breaks');
  console.log('='.repeat(60));
